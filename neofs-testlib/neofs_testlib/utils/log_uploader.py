...
NEOFS_CLI_VERSION = "v0.53.0"
NEOFS_CLI_URL = f"https://github.com/nspcc-dev/neofs-node/releases/download/{NEOFS_CLI_VERSION}/neofs-cli-linux-amd64"
...
class NeofsUploader:
    ...
    def _ensure_cli(self) -> str:
        ...
        if not cli_path.exists():
            ...
            self._run(f"wget -q -O {cli_path} {NEOFS_CLI_URL}", timeout=self.DOWNLOAD_TIMEOUT)
            self._run(f"chmod +x {cli_path}", timeout=30)
