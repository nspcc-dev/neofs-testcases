...
pytest-local:
	@echo "⇒ Run Pytest"
	export PYTHONPATH=$(ROOT_DIR)/neofs-keywords/lib:$(ROOT_DIR)/neofs-keywords/robot:$(ROOT_DIR)/robot/resources/lib:$(ROOT_DIR)/robot/resources/lib/python_helpers:$(ROOT_DIR)/robot/variables && \
...
pytest-docker:
	...
	docker run -t --rm                                  \
		--name neofs_tests_py                           \
		-e PYTHONPATH="/root/neofs-keywords/lib:/root/neofs-keywords/robot:/root/robot/resources/lib:/root/robot/resources/lib/python_helpers:/root/robot/variables"			\
		$(PY_VARIABLES)									\
		-v $(CURDIR):/root			 			        \
