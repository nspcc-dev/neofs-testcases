...
def test_object_revival(neofs_env_single_sn_custom_gc: NeoFSEnv, default_wallet: NodeWallet, object_size: str):
    neofs_env = neofs_env_single_sn_custom_gc
    allure.dynamic.title(f"Verify that an object of {object_size} can be revived")
    ...
    created_objects = []
    for _ in range(2):
        file_path = generate_file(neofs_env.get_object_size(object_size))
