...
def test_put_object_without_homo_hash(default_wallet: NodeWallet, neofs_env: NeoFSEnv):
    ...
    with allure.step("Try to put an object via grpc without homo hash"):
        response = put_object(
            object_types_pb2.ObjectType.REGULAR, default_wallet, neofs_env, b"payload", cid, homo_hash=False
        )
