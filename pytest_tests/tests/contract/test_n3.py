...
def get_container_marshalled_bytes(datadir: str, template_file_name: str) -> bytes:
    """
    ...
    owner_bytes = base58.b58decode(request_json["Container"]["Owner"])
    ...
    flt = container.placement_policy.filters.add()
    flt.key = "Deployed"
    flt.value = "NSPCC"
    flt.name = "F"
    flt.op = 1  # EQ
...
def render_create_container_json(
    datadir: str, template_file_name: str, verification_script_b64: str, invocation_script_b64: str
) -> None:
    jinja_env = jinja2.Environment()
    template = Path(os.path.join(datadir, template_file_name)).read_text()
    jinja_template = jinja_env.from_string(template)
    rendered_config = jinja_template.render(
        verification_script=verification_script_b64, invocation_script=invocation_script_b64
    )
    ...
    def test_n3_contract_witnesses_in_container_ops(self, datadir, neofs_env: NeoFSEnv):
        ...
        with allure.step("Calculate invocation script"):
            container_marshalled_bytes = get_container_marshalled_bytes(
                datadir, CONTAINER_REQUEST_TEMPLATE_JSON_FILE_NAME
            )
        ...
        with allure.step("Create container"):
            render_create_container_json(
                datadir, CONTAINER_REQUEST_TEMPLATE_JSON_FILE_NAME, verification_script_b64, invocation_script_b64
            )
        ...
        with allure.step("Create container with wrong user"):
            invalid_container_marshalled_bytes = get_container_marshalled_bytes(
                datadir, CONTAINER_INVALID_REQUEST_TEMPLATE_JSON_FILE_NAME
            )
