...
def test_rest_gw_metrics(neofs_env_single_sn: NeoFSEnv, default_wallet: NodeWallet):
    ...
    session_token = generate_session_token_v2(
        gw_endpoint, default_wallet, [{"verbs": ["CONTAINER_PUT"]}], wallet_connect=True
    )
    ...
    session_token = generate_session_token_v2(
        gw_endpoint, default_wallet, [{"verbs": ["CONTAINER_DELETE"]}], wallet_connect=True
    )
