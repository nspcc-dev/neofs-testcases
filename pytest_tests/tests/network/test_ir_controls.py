...
def wait_until_new_config_value(neofs_env: NeoFSEnv, config_key: str, config_value: int) -> int:
    net_info = get_netmap_netinfo(
        ...
        endpoint=neofs_env.storage_nodes[0].rpc_endpoint,
...
def wait_until_node_disappears_from_netmap_snapshot(
    ...
    netmap_snapshot = (
        ...
        .netmap.snapshot(
            rpc_endpoint=alive_node.rpc_endpoint,
...
def test_control_notary_request_node_removal(neofs_env_4_ir_4_sn: NeoFSEnv):
    ...
    with allure.step("Stop storage node before notary request finalization"):
        sn_offline_node.stop()
    ...
    try:
        ...
        with allure.step("Node should disappear in the next epoch"):
            neofs_epoch.ensure_fresh_epoch(neofs_env, alive_node)
            wait_until_node_disappears_from_netmap_snapshot(neofs_env, alive_node, sn_offline_node_addr)
    finally:
        if sn_offline_node.process is None:
            start_storage_nodes([sn_offline_node])
