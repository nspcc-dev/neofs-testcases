...
import allure
import pytest
from helpers.acl import EACLRole
...
def wallets(default_wallet, temp_directory, neofs_env: NeoFSEnv) -> Wallets:
    default_wallet_config_path = neofs_env._generate_temp_file(temp_directory, extension="yml")
    ...
    other_wallet1_config_path = neofs_env._generate_temp_file(temp_directory, extension="yml")
    NeoFSEnv.generate_config_file(
        config_template="cli_cfg.yaml", config_path=other_wallet1_config_path, wallet=other_wallet1
    )

    other_wallet2 = create_wallet()
    other_wallet2_config_path = neofs_env._generate_temp_file(temp_directory, extension="yml")
    NeoFSEnv.generate_config_file(
