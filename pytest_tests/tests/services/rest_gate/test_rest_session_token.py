...
from rest_gw.rest_utils import extract_session_token_from_bearer, generate_session_token_v2, sign_session_token
...
class TestRestSessionTokenV2(TestNeofsRestBase):
    ...
    def test_rest_v2_session_token_expiration(self, gw_endpoint: str):
        ...
        2. Create V2 session token with short lifetime (30 seconds)
    ...
    def test_rest_v2_session_token_nns_delegation(self, gw_endpoint: str):
        ...
        with allure.step("Refill gas for owner wallet"):
            neofs_env.neofs_adm().fschain.refill_gas(
                rpc_endpoint=f"http://{neofs_env.fschain_rpc}",
                alphabet_wallets=neofs_env.alphabet_wallets_dir,
                storage_wallet=self.user_wallet.path,
                gas=200,
                wallet_address=self.user_wallet.address,
            )
        ...
        with allure.step("Register NNS domains"):
            ...
            register_nns_domain_with_record(
                ...
                domain=user_domain,
                contracts_hashes=contracts_hashes,
                neo_address=self.user_wallet.address,
            )

        with allure.step("Verify domains are registered"):
            raw_dumped_names = neofs_env.neofs_adm().fschain.dump_names(f"http://{neofs_env.fschain_rpc}").stdout
            assert user_domain in raw_dumped_names
    ...
    @pytest.mark.simple
    def test_rest_v2_session_token_without_lock(self, gw_endpoint: str):
        """
        Test that session token without lock (raw session token) is rejected.

        Steps:
        1. Create container and upload object
        2. Create V2 session token
        3. Extract session token part (without lock)
        4. Attempt to use extracted token directly
        ...
        with allure.step("Create V2 Session Token for GET operation"):
            contexts = [{"containerID": cid, "verbs": ["OBJECT_GET"]}]
            session_token = generate_session_token_v2(
                gw_endpoint, self.owner_wallet, contexts, targets=[rest_gw_address]
            )

        with allure.step("Extract session token part without lock"):
            extracted_token = extract_session_token_from_bearer(session_token)

        with allure.step("Attempt to use token without lock (should fail)"):
            resp = get_via_rest_gate(cid, oid, gw_endpoint, session_token=extracted_token, expect_error=True)
            assert not resp.ok, "Request with session token without lock should be rejected"

    @allure.title("Test V2 Session Token Security - Tampered Lock")
    @pytest.mark.simple
    def test_rest_v2_session_token_tampered_lock(self, gw_endpoint: str):
        """
        Test that session token with tampered lock is rejected.

        Steps:
        1. Create container and upload object
        2. Create V2 session token
        3. Replace lock with different 32 bytes
        4. Attempt to use token with tampered lock
        5. Verify request is rejected
        """
        with allure.step("Create container"):
            container_token = generate_session_token_v2(gw_endpoint, self.owner_wallet, [{"verbs": ["CONTAINER_PUT"]}])
        ...
        with allure.step("Tamper with lock (replace with random 32 bytes)"):
            bearer_bytes = base64.standard_b64decode(session_token)
            tampered_lock = os.urandom(32)
            session_token_part = bearer_bytes[32:]
            tampered_bearer = tampered_lock + session_token_part
            tampered_token = base64.standard_b64encode(tampered_bearer).decode("utf-8")

        with allure.step("Attempt to use token with tampered lock (should fail)"):
            resp = get_via_rest_gate(cid, oid, gw_endpoint, session_token=tampered_token, expect_error=True)
            assert not resp.ok, "Request with tampered lock should be rejected"

    @allure.title("Test V2 Session Token Security - Invalid Signature")
    @pytest.mark.simple
    def test_rest_v2_session_token_invalid_signature(self, gw_endpoint: str):
        """
        Test that session token with invalid signature is rejected.

        Steps:
        1. Create container
        2. Get unsigned session token
        3. Sign with wrong wallet (stranger wallet)
        4. Attempt to use incorrectly signed token
        5. Verify request is rejected
        """
        with allure.step("Create container"):
            container_token = generate_session_token_v2(gw_endpoint, self.owner_wallet, [{"verbs": ["CONTAINER_PUT"]}])
        ...
        with allure.step("Get unsigned session token for owner wallet"):
            owner_neo3_wallet = load_wallet(self.owner_wallet.path, self.owner_wallet.password)
            owner_acc = owner_neo3_wallet.accounts[0]

            contexts = [{"containerID": cid, "verbs": ["OBJECT_GET"]}]
            unsigned_token, lock = get_unsigned_session_token(
                gw_endpoint,
                issuer=owner_acc.address,
                contexts=contexts,
                lifetime=100,
                targets=[rest_gw_address],
            )

        with allure.step("Sign token with wrong wallet (stranger wallet)"):
            stranger_neo3_wallet = load_wallet(self.stranger_wallet.path, self.stranger_wallet.password)
            stranger_acc = stranger_neo3_wallet.accounts[0]
        ...
        with allure.step("Complete token with wrong signature"):
            try:
                invalid_token = complete_session_token(
        ...
        with allure.step("Attempt to use token with invalid signature (should fail)"):
            resp = get_via_rest_gate(cid, oid, gw_endpoint, session_token=invalid_token, expect_error=True)
            assert not resp.ok, "Request with invalid signature should be rejected"

    @allure.title("Test V2 Session Token Security - Tampered Token Body")
    @pytest.mark.simple
    def test_rest_v2_session_token_tampered_body(self, gw_endpoint: str):
        """
        Test that session token with tampered body is rejected.

        Steps:
        1. Create two containers
        2. Upload objects to both containers
        3. Create V2 session token for first container only
        4. Tamper with token to try to use it for second container
        5. Verify request is rejected
        """
        with allure.step("Create containers"):
            container_token = generate_session_token_v2(gw_endpoint, self.owner_wallet, [{"verbs": ["CONTAINER_PUT"]}])
        ...
        with allure.step("Upload objects"):
            rest_gw_address = get_rest_gateway_address(gw_endpoint)
            upload_token = generate_session_token_v2(
                gw_endpoint, self.owner_wallet, [{"verbs": ["OBJECT_PUT"]}], targets=[rest_gw_address]
            )
        ...
        with allure.step("Create V2 Session Token for first container only"):
            contexts = [{"containerID": cid1, "verbs": ["OBJECT_GET"]}]
            session_token = generate_session_token_v2(
                gw_endpoint, self.owner_wallet, contexts, targets=[rest_gw_address]
            )
        ...
        with allure.step("Verify token works for first container"):
            resp = get_via_rest_gate(cid1, oid1, gw_endpoint, session_token=session_token, return_response=True)
            assert resp.ok, "Token should work for authorized container"

        with allure.step("Attempt to use token for second container (should fail)"):
            resp = get_via_rest_gate(cid2, oid2, gw_endpoint, session_token=session_token, expect_error=True)
            assert not resp.ok, "Token should be rejected for unauthorized container"
