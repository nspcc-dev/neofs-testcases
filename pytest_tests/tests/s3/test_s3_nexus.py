...
def configure_blob_store(
    ...
    data = {
        ...
        "type": "S3",
        "bucketConfiguration": {
            "bucket": {"region": "DEFAULT", "name": s3_bucket_name, "prefix": "", "expiration": 0},
            "bucketSecurity": {
...
class TestS3Nexus(TestNeofsS3Base):
    ...
    @pytest.mark.skip(reason="Nexus is not supported right now")
    @pytest.mark.skipif(sys.platform == "darwin", reason="not supported on macos runners")
    def test_s3_nexus_configure_blob_store(self, bucket):
