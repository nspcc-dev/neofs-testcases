...
def _sign_token_payload(
    ...
    """
    Sign a raw token payload using either deterministic SHA256 or WalletConnect.

    Args:
        payload: Raw bytes to sign (e.g. base64-decoded unsigned token body).
        private_key: Private key to sign with.
        wallet_connect: If True, wrap the payload into the WalletConnect message
                        envelope (prefix/length/salt/postfix), sign that envelope
                        and append the salt to the signature. Otherwise sign the
                        payload directly with deterministic ECDSA SHA256.

    Returns:
        tuple: (raw_signature_bytes, scheme_name) where for WalletConnect the
        signature already has the salt appended (the verifier extracts it the
        same way).
    """
    if wallet_connect:
        prefix = b"\x01\x00\x01\xf0"
        postfix = b"\x00\x00"
        encoded_payload = base64.standard_b64encode(payload)
        salt = os.urandom(16)
        hex_salt = binascii.hexlify(salt)
        msg_len = len(hex_salt) + len(encoded_payload)
        msg = prefix + msg_len.to_bytes() + hex_salt + encoded_payload + postfix
        signature_bytes = cryptography.sign(msg, private_key, hash_func=hashlib.sha256)
        return signature_bytes + salt, "WALLETCONNECT"

    signature_bytes = cryptography.sign(payload, private_key, hash_func=hashlib.sha256)
    return signature_bytes, "DETERMINISTIC_SHA256"
...
def sign_session_token(
    ...
    raw_signature, scheme = _sign_token_payload(unsigned_token_bytes, private_key, wallet_connect=wallet_connect)

    signature = base64.standard_b64encode(raw_signature).decode("utf-8")
...
def sign_bearer_token_v2(
    ...
    signature = binascii.hexlify(raw_signature).decode("utf-8")
    pub_key = binascii.hexlify(public_key_bytes).decode("utf-8")
